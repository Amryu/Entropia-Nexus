...
function register(app){
   ...
   */
  app.get('/capsules', async (req,res) => { res.json(await withCache('/capsules', ['Capsules'], getCapsules)); });
  /**
   * @swagger
   * /capsules/{capsule}:
   *  get:
   ...
   */
  app.get('/capsules/:capsule', async (req,res) => { const r = await withCachedLookup('/capsules', ['Capsules'], getCapsules, req.params.capsule); if (r) res.json(r); else res.status(404).send(); });
}
