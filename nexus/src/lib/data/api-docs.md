...
Crowdsourced trade messages and global events. The distribution (GET) endpoints are **public** — no authentication required, rate limited by IP. Submission (POST) endpoints are restricted to approved OAuth applications only — the application's `client_id` must be whitelisted by an administrator. If your application is not approved, POST requests will return `403`.
