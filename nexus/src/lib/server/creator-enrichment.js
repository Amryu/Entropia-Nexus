...
async function fetchYouTubeData(channelId) {
  ...
  // 1. RSS feed for latest video (free, no API key needed)
  try {
    const rssUrl = `https://www.youtube.com/feeds/videos.xml?channel_id=${channelId}`;
    const rssResponse = await fetch(rssUrl);
    if (rssResponse.ok) {
      const xml = await rssResponse.text();
      const videoMatch = xml.match(/<entry>[\s\S]*?<yt:videoId>([^<]+)<\/yt:videoId>[\s\S]*?<title>([^<]+)<\/title>[\s\S]*?<published>([^<]+)<\/published>/);
      if (videoMatch) {
        data.latestVideo = {
          videoId: videoMatch[1],
          title: videoMatch[2],
          thumbnail: `https://i.ytimg.com/vi/${videoMatch[1]}/mqdefault.jpg`,
          publishedAt: videoMatch[3]
        };
      }
    }
  } catch (err) {
