...
import { resolveItemIdByName } from '$lib/server/item-type-cache.js';
...
export async function POST({ request, locals, fetch }) {
  ...
  try {
    ...
    const resolveId = (name) => resolveItemIdByName(name, fetch);
    const result = await ingestMarketPrices(user.id, validPrices, resolveId);
