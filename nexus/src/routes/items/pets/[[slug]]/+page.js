...
export async function load({ fetch, params, url, parent }) {
  if (url.searchParams.get('mode') === 'view') {
    redirect(301, `/items/pets/${encodeURIComponentSafe(params.slug)}`);
  }
  ...
  // Fetch planets and effects for edit mode dropdowns
  const [planetsList, effectsList] = await Promise.all([
    apiCall(fetch, '/planets'),
    apiCall(fetch, '/effects')
  ]);
  response.planets = planetsList || [];
