<!--
  @component Top Targets Page
  Paginated, filterable ranking of targets by global event statistics.
-->
<script>
  // @ts-nocheck
  ...
  let sortBy = $state(sp.get('sort') || 'count');
  let groupBy = $state('mob');
...
<div class="ranking-page">
    ...
    <div class="sort-toggles">
      {#if isHuntingFilter}
        <div class="sort-toggle">
          <button class="sort-btn" class:active={groupBy === 'maturity'} onclick={() => onGroupChange('maturity')}>Maturities</button>
          <button class="sort-btn" class:active={groupBy === 'mob'} onclick={() => onGroupChange('mob')}>Mobs</button>
        </div>
      {/if}
      <div class="sort-toggle">
  ...
  {:else if !data || data.targets.length === 0}
    <p class="empty-state">No targets match your filters</p>
  {:else}
    <div class="table-wrapper">
      <table class="data-table">
        ...
        <tbody>
          {#each data.targets as t, i}
            {@const tc = getTypeConfig(t.primary_type)}
            <tr>
              <td class="col-rank text-muted">{(currentPage - 1) * 50 + i + 1}</td>
              <td>
                <a href="/globals/target/{encodeURIComponent(t.target)}" class="target-link">{t.target}</a>
              </td>
