...
<script>
  ...
  function selectOwner(u) {
    // Store only the ID in Properties.OwnerId (as the schema expects)
    updateField('Properties.OwnerId', u ? Number(u.id) : null);
...
<style>
  ...
  /* Show on Map button - styled like shops page */
  .map-link-btn {
    display: flex;
    align-items: center;
    justify-content: center;
