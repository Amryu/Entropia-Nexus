    ...
    if (!thread) {
      ...
      if (entityPath) {
        try {
          ...
          const locationTypes = ['Location', 'Area', 'Apartment'];
          const mode = locationTypes.includes(change.entity) || change.type !== 'Create' ? 'edit' : 'create';
          const editUrl = `${baseUrl}?mode=${mode}&changeId=${change.id}`;
