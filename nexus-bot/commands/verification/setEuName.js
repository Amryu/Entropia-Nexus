  ...
  try {
    ...
    if (moderatorRoleId) {
      ...
      if (moderatorRole) {
        ...
        const parentChannelId = interaction.channel.parentId || threadId;
        const threadLink = `https://discord.com/channels/${guildId}/${threadId}`;
        const message = [
          `A user with the ID ${interaction.user.id} has set their EU name to "${euName}". Please verify their account.`,
          `Thread: ${threadLink}`,
          `Guild ID: ${guildId}`,
        ...
        await Promise.all(
          moderatorRole.members.map(member => member.send(message).catch(err => {
    ...
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({ content: 'An error occurred while processing your request. Please try again later.', ephemeral: true }).catch(() => {});
    }
  }
}
