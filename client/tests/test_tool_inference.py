...
class TestToolInferenceEngine(unittest.TestCase):
    ...
    # --- Buffering and enrichment ---

    def test_buffer_and_enrich(self):
        now = datetime.utcnow()
        ...
        self.engine.buffer_event(event)

        enriched = self.engine.enrich_buffered("Gun", "ocr")
        self.assertEqual(len(enriched), 1)
        self.assertEqual(enriched[0].tool_name, "Gun")
        self.assertEqual(enriched[0].tool_source, "ocr")
        self.assertAlmostEqual(enriched[0].inferred_confidence, 1.0)

    def test_enrich_non_ocr_confidence(self):
        now = datetime.utcnow()
        event = self.engine.create_event(
            encounter_id="enc-1", timestamp=now,
            event_type="damage_dealt", amount=50.0,
        ...
        self.engine.buffer_event(event)
        enriched = self.engine.enrich_buffered("Gun", "inferred")
        self.assertAlmostEqual(enriched[0].inferred_confidence, 0.8)
