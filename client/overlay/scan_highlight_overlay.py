...
class ScanHighlightOverlay(QWidget):
    ...
    def _paint_target_lock_rois(self, painter: QPainter, tx: int, ty: int) -> None:
        ...
        # Show the name crop (what ONNX sees) above the name ROI
        data = self._target_lock_data
        if not data:
            return
        ...
        if name_crop is not None and name_roi:
            try:
                ...
                crop_y = ty + name_roi.get("dy", 0) - disp_h - 2
                painter.drawPixmap(crop_x, crop_y, pixmap)
                painter.setPen(QPen(QColor(255, 255, 255, 200)))
                painter.setFont(TARGET_LOCK_LABEL_FONT)
                painter.drawText(crop_x, crop_y - 2, f"→ {raw_name!r}")
            except Exception:
                pass
        elif raw_name:
            painter.setPen(QPen(QColor(255, 255, 255, 230)))
            painter.setFont(TARGET_LOCK_LABEL_FONT)
            painter.drawText(tx - 100, ty - 50, f"→ {raw_name!r}")
