...
def _section_title_label(text: str) -> QLabel:
...
def _make_compact_table(headers: list[str], rows: list[list[str]]) -> QTableWidget:
...
class WeaponDetailView(WikiDetailView):
    ...
    def _build_acquisition_content(self, data: dict) -> QWidget:
        ...
        if vendor_offers:
            has_any = True
            layout.addWidget(_section_title_label("Vendor"))
            ...
            layout.addWidget(_make_compact_table(headers, rows))

        # --- Looted ---
        loots = data.get("Loots") or []
        if loots:
            has_any = True
            layout.addWidget(_section_title_label("Looted"))
        ...
        if shop_listings:
            has_any = True
            layout.addWidget(_section_title_label("Market"))
        ...
        if blueprints:
            has_any = True
            layout.addWidget(_section_title_label("Crafted"))
        ...
        if bp_drops:
            has_any = True
            layout.addWidget(_section_title_label("Blueprint Discovery"))
            headers = ["Name", "Level"]
            rows = []
            for bp in bp_drops:
                bp_name = bp.get("Name", "N/A")
        ...
        if not has_any:
            layout.addWidget(
                _no_data_label("No acquisition data available for this item.")
            )
